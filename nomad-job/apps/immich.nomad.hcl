job "immich" {
  ...
  group "immich" {
    ...
    task "immich-server" {
      ...
      config {
        image   = "ghcr.service.consul:5000/immich-app/immich-server:v1.140.1"
        ports   = ["http"]
        volumes = [
    ...
    task "immich-machine-learning" {
      ...
      config {
        image = "ghcr.service.consul:5000/immich-app/immich-machine-learning:v1.140.1"
    ...
    task "redis" {
      driver = "docker"
      config {
        image="docker.service.consul:5000/library/redis:8.2-alpine"
        ports = ["redis"]
      }
      resources {
        memory = 50

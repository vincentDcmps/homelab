  ...
  group "forjego" {
    ...
    task "forjego" {
      ...
      env {
        ...
        GITEA__database__NAME                            = "forjego"
        GITEA__database__USER                            = "forjego"
      ...
      template {
        data        = <<EOH
        {{ with secret "secrets/data/nomad/forjego"}}
          ...
          {{ with secret "secrets/data/database/forjego"}}
    ...
    task "anubis" {
      ...
      service {
        ...
        tags = [
          "traefik.enable=true",
          "traefik.enable=true",
          "traefik.http.routers.anubis.rule=Host(`git.ducamps.eu`)",
          "traefik.http.routers.anubis.tls.domains[0].sans=git.ducamps.eu",
          "traefik.http.routers.anubis.tls.certresolver=myresolver",
          "traefik.http.middlewares.httpsRedirect.redirectscheme.scheme=https",
          "traefik.http.routers.anubis.middlewares=httpsRedirect",
          "traefik.http.routers.anubis.entrypoints=web,websecure",
        ]
      }
      config {
        image = "ghcr.service.consul:5000/techarohq/anubis:v1.25.0"
        ports = [
          "anubis"
        ]
        volumes = [
          "local/gitea.botPolicies.yaml:/etc/anubis/gitea.botPolicies.yaml",

        ]
      }
      env {
        BIND             = ":8080"
        DIFFICULTY       = "4" # Fast successful response with most devices, 5 is way more slow
        SERVE_ROBOTS_TXT = "false"
        POLICY_FNAME     = "/etc/anubis/gitea.botPolicies.yaml"
        TARGET           = "http://traefik-local.service.consul:3443"
      }
      template {
        data        = <<EOH
bots:
...
thresholds:
  ...
  - name: extreme-suspicion
    ...
    challenge:
      ...
      report_as: 4
          EOH
        destination = "local/gitea.botPolicies.yaml"
      }
